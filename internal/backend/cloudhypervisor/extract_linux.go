...
// only by the linux backend binary (the seam stays library-agnostic).
...
// bootArgs returns a DIRECT KERNEL boot for cloud-hypervisor on both arches: it
// extracts the image's own kernel+initrd once (cached in the VM dir next to
// disk.raw) and hands them to cloud-hypervisor with the per-arch bootCmdline.
// Direct boot is the path cloud-hypervisor supports first-class and which boots
// reliably across the whole catalog; the boot methods it replaced, and why each
// was inadequate, are recorded in ADR-0024 (arm64) and ADR-0029 (x86_64).
//
// Tradeoff: the extracted kernel is the image's kernel at first boot; a guest that
// later updates its own kernel still boots the original until rm+up. Acceptable for
// cattle VMs (noted in ADR-0024).
