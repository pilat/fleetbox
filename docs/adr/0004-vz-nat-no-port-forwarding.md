...
- **Discovered after the spec was written: VM→VM traffic does NOT work over VZ NAT.**
  ...
  them. v0 therefore targets single-VM testing; multi-node cluster testing is deferred
  to v1, where bridged networking or socket_vmnet are the candidate fixes. This is the
  main known limitation of v0 (see `ARCHITECTURE.md §7`).
